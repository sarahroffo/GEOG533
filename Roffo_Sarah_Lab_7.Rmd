...
Solution 1b
```{r}
cor.test(income, edu, method = "pearson")
...
Solution 1d
```{r}
cor.test(income, edu, alternative = "greater", method = "spearman")
print('The observed value of rs is greater than 0')
```
...
t <- qt(0.975, df = 34)
t/(sqrt(34) + t)
##Smallest corellation coefficient to be significant for 36 sample size is 
t <- qt(0.975, df = 78)
t/(sqrt(78) + t)
##Smallest corellation coefficient to be significant for 80 sample size is 
```
...
### Solution
```{r}
cor(income, races, method = "pearson")
#correlation coefficient
cor.test(income, races, method = "pearson")
print('We reject the Null hypothesis that the true correlation coefficient is eaual to 0')
```
...
#Reject Null if p-value > .10
print('We reject the Null hypothesis that the true correlation is equal to zero')
```
...
#Reject null of p-value > 0.05
print('We reject the null hypothesis that the correlation coefficient is equal to zero')
```
...
### Solution
```{r}
M <- read.csv(file = "Milwaukee_Sales.csv")
Bedrooms <- M$Bedrms
Lot <- M$LotSize
cor(Bedrooms, Lot, method = "pearson")
```
...
### Solution
```{r}
UK <- read.csv(file = "UK_Housing.csv")
Floor <- UK$floorarea
bedroom <- UK$bedrooms
cor(Floor, bedroom, method = "pearson")
```
...
```{r}
cor.test(x, y, method = "pearson")
#Pearson's correlation coefficient using cor.test() and 'r' from part c are the same.
```
...
Solution 9f
```{r}
cor.test(rank(df$speed), rank(df$dist), method = "spearman")
#Spearman's rank correlation coefficient is about the same as 'rs' from part e.
```
